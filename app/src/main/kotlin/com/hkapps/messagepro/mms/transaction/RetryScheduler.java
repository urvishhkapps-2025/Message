    ...
    private void markMmsFailed(final Context context) {
        ...
        try {
            ...
            BroadcastUtils.sendExplicitBroadcast(
                    mContext, new Intent(), com.hkapps.messagepro.klinker.android.send_message.Transaction.REFRESH);
            BroadcastUtils.sendExplicitBroadcast(
                    mContext,
                    new Intent(),
                    com.hkapps.messagepro.klinker.android.send_message.Transaction.NOTIFY_SMS_FAILURE);

            // broadcast that mms has failed and you can notify user from there if you would like
            BroadcastUtils.sendExplicitBroadcast(
                    mContext, new Intent(), com.hkapps.messagepro.klinker.android.send_message.Transaction.MMS_ERROR);
        } catch (Exception e) {
        }
    }

...
class SimpleContactsHelperUtils(val context: Context) {
    fun getAvailableContacts(favoritesOnly: Boolean, callback: (ArrayList<ContactsModel>) -> Unit) {
        ensureBackgroundThread {
            ...
            }.distinctBy { it.rawId }.toMutableList() as ArrayList<ContactsModel>
            ...
            val contactsToRemove = ArrayList<ContactsModel>()
            allContacts.groupBy { it.name }.forEach {
                val contacts = it.value.toMutableList() as ArrayList<ContactsModel>
                if (contacts.size > 1) {
    ...
    private fun getContactNames(favoritesOnly: Boolean): List<ContactsModel> {
        val contacts = ArrayList<ContactsModel>()
        val startNameWithSurname = context.mPref.startNameWithSurname
        ...
        context.queryCursor(uri, projection, selection, selectionArgs) { cursor ->
            ...
            if (isPerson) {
                ...
                if (firstName.isNotEmpty() || middleName.isNotEmpty() || familyName.isNotEmpty()) {
                    ...
                    val contact = ContactsModel(rawId, contactId, fullName, photoUri, ArrayList(), ArrayList(), ArrayList())
    ...
    private fun getContactPhoneNumbers(favoritesOnly: Boolean): ArrayList<ContactsModel> {
        val contacts = ArrayList<ContactsModel>()
        ...
        context.queryCursor(uri, projection, selection) { cursor ->
            ...
            if (contacts.firstOrNull { it.rawId == rawId } == null) {
                val contact = ContactsModel(rawId, contactId, "", "", ArrayList(), ArrayList(), ArrayList())
                contacts.add(contact)
            }
    ...
    fun getContactLetterIcon(name: String): Bitmap {
        ...
        val size = context.resources.getDimension(R.dimen.margin_48).toInt()
    ...
    fun getColoredGroupIcon(context: Context,title: String): Drawable {
        val icon = ContextCompat.getDrawable(context,R.drawable.ic_group_circle_bg)
    ...
    fun getShortcutImage(path: String, placeholderName: String, callback: (image: Bitmap) -> Unit) {
        ensureBackgroundThread {
            ...
            try {
                ...
                val size = context.resources.getDimension(R.dimen.margin_64).toInt()

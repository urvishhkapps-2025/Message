...
import com.hkapps.messagepro.klinker.android.send_message.Utils
import com.hkapps.messagepro.extensions.getLongValue
import com.hkapps.messagepro.extensions.queryCursor
import com.hkapps.messagepro.model.MMSModel
import com.hkapps.messagepro.model.MMSBackupModel
import com.hkapps.messagepro.model.MMSPartModel
import com.hkapps.messagepro.model.SmsBackupModel

class WriterUtils(private val context: Context) {
    private val INVALID_ID = -1L
    private val contentResolver = context.contentResolver

    fun writeSmsMessage(smsBackup: SmsBackupModel) {
    ...
    private fun smsExist(smsBackup: SmsBackupModel): Boolean {
    ...
    private fun getMmsThreadId(mmsBackup: MMSBackupModel): Long {
    ...
    private fun getMmsId(mmsBackup: MMSBackupModel): Long {
        val threadId = getMmsThreadId(mmsBackup)
    ...
    private fun mmsExist(mmsBackup: MMSBackupModel): Boolean {
        return getMmsId(mmsBackup) != INVALID_ID
    }

    @SuppressLint("NewApi")
    private fun mmsAddressExist(mmsAddress: MMSModel, messageId: Long): Boolean {
    ...
    @SuppressLint("NewApi")
    private fun writeMmsAddress(mmsAddress: MMSModel, messageId: Long) {
    ...
    @SuppressLint("NewApi")
    private fun writeMmsPart(mmsPart: MMSPartModel, messageId: Long) {
    ...
    @SuppressLint("NewApi")
    private fun mmsPartExist(mmsPart: MMSPartModel, messageId: Long): Boolean {
        val uri = Uri.parse("content://mms/${messageId}/part")

...
class ReaderUtils(private val context: Context) {
    fun forEachSms(threadId: Long, block: (SmsBackupModel) -> Unit) {
        ...
        context.queryCursor(Sms.CONTENT_URI, projection, selection, selectionArgs) { cursor ->
            ...
            block(SmsBackupModel(subscriptionId, address, body, date, dateSent, locked, protocol, read, status, type, serviceCenter))
        }
    }

    // all mms from simple sms are non-text messages
    fun forEachMms(threadId: Long, includeTextOnlyAttachment: Boolean = false, block: (MMSBackupModel) -> Unit) {
        val projection = arrayOf(
            Mms._ID,
            Mms.CREATOR,
            Mms.CONTENT_TYPE,
    ...
    @SuppressLint("NewApi")
    private fun getParts(mmsId: Long): List<MMSPartModel> {
        ...
        context.queryCursor(uri, projection, selection, selectionArgs) { cursor ->
            ...
            parts.add(MMSPartModel(contentDisposition, charset, contentId, contentLocation, contentType, ctStart, ctType, filename, name, sequenceOrder, text, data))
        }
        return parts
    }
    ...
    @SuppressLint("NewApi")
    private fun getMmsAddresses(messageId: Long): List<MMSModel> {
        val addresses = mutableListOf<MMSModel>()
        ...
        context.queryCursor(uri, projection, selection, selectionArgs) { cursor ->
            ...
            addresses.add(MMSModel(address, type, charset))
        }
        return addresses
    }

...
import com.hkapps.messagepro.utils.NumbersUtils
import java.io.File
import java.util.Locale
import java.util.Locale.getDefault

open class FileDIRModel(
    val path: String,
    val name: String = "",
    ...
    override fun compareTo(other: FileDIRModel): Int {
            ...
            when {
                sorting and SORT_BY_NAME != 0 -> {
                    result = if (sorting and SORT_USE_NUMERIC_VALUE != 0) {
                        NumbersUtils().compare(name.lowercase(getDefault()), other.name.lowercase(getDefault()))
                    } else {
                        name.lowercase(getDefault()).compareTo(other.name.lowercase(getDefault()))
                    }
                }
                ...
                sorting and SORT_BY_DATE_MODIFIED != 0 -> {
                    ...
                    result = getExtension().lowercase(getDefault()).compareTo(other.getExtension().lowercase(getDefault()))
                }
            }
    ...
    fun getBubbleText(context: Context, dateFormat: String? = null, timeFormat: String? = null) = when {
        ...
        sorting and SORT_BY_EXTENSION != 0 -> getExtension().lowercase(getDefault())
        else -> name
    }
